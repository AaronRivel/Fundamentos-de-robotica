...
def simulacion(q0,tf,h):
    ...
    def f(t,q):
        ...
        for i in range(len(q0)):
            Jv = J[:3][i]
            Jw = J[3:6][i]
            #D = D + np.dot(m,np.transpose(Jv)@Jv + np.transpose(Jw)@T[i+1][:3,:3]@I@np.transpose(T[i+1][:3,:3])@Jw)
            #c_vector[i][:] = T[i][:3,3]+(T[i+1][:3,3]-T[i][:3,3])*0.5
        #G[0] = np.dot(np.dot(m,g),[0,0,0]) + np.dot(np.dot(m,g),[-np.sin(q[0])*1,np.cos(q[0])*1,1]) + np.dot(np.dot(m,g),[-np.sin(q[0])*1,np.cos(q[0])*1,1])
        #G[1] = np.dot(np.dot(m,g),[-np.sin(q[0])*1,np.cos(q[0])*1,1])
        '''
        global e_anterior
        D = np.identity(3)
        R, Rv=Ref(t)
...
e_anterior = 0
q0 = np.array([0,0,0])
I = np.identity(len(q0))
tf=10
h=0.05
J = np.zeros((6,len(q0)))
#Jc = np.zeros((6,len(q0)))

#c_vector = np.zeros((len(q0),3))
